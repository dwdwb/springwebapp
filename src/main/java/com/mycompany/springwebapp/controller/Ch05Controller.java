...
public class Ch05Controller {
	...
	public String createCookie(HttpServletResponse response) {
		...
		cookie.setPath("/");  			//쿠키를  재전송할 경로 지정 (/: 서버 모든 곳)
		cookie.setMaxAge(30*60);  		//쿠키의 저장 기간(단위: 초, 파일에 저장), 시간을 지정 안할 경우: 브라우저 메모리에 저장(끄면 날라감)
		cookie.setHttpOnly(false);  	//true: 서버만 이용, false: JavaScript에서 접근 허용
		cookie.setSecure(false);  		//false: http, https 모두 쿠키 사용, true: https만 사용
